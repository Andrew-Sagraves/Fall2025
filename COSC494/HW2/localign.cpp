...
void fill(int **grid, char **tb, int rows, int columns, const string &one,
          const string &two, int &maxx, int &maxy) {
  int max = 0;
  for (int i = 1; i < rows; i++) {
    for (int j = 1; j < columns; j++) {
...
// traceback alignment
void traceback(char **tb, int x, int y, const string &one, const string &two,
...
int main(int argc, char *argv[]) {
  ...
  fill(grid, tb, rows, columns, one, two, startx, starty);

  cout << "Alignment score: " << grid[startx][starty] << '\n';

  string aln1, aln2;
  traceback(tb, startx, starty, one, two, aln1, aln2);
